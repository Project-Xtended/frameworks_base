...
public class SleepModeController {
    ...
    private static int mAggresiveIdleBatteryState;
    private static int mAggresiveStandbyBatteryState;
    ...
    private void enable() {
        ...
        // Enable Aggressive Battery Saver
        boolean enableAggressiveBat = Settings.Secure.getIntForUser(mContext.getContentResolver(),
                Settings.Secure.SLEEP_MODE_AGGRESSIVE_BATTERY_TOGGLE, 1, UserHandle.USER_CURRENT) == 1;
        if (enableAggressiveBat) {
            mAggresiveIdleBatteryState = Settings.Global.getInt(mContext.getContentResolver(),
                 Settings.Global.AGGRESSIVE_IDLE_ENABLED, 1);

            Settings.Global.putInt(mContext.getContentResolver(), Settings.Global.AGGRESSIVE_IDLE_ENABLED, 1);

            mAggresiveStandbyBatteryState = Settings.Global.getInt(mContext.getContentResolver(),
                 Settings.Global.AGGRESSIVE_STANDBY_ENABLED, 1);

            Settings.Global.putInt(mContext.getContentResolver(), Settings.Global.AGGRESSIVE_STANDBY_ENABLED, 1);
        }
    ...
    private void disable() {
        ...
        if (enableAggressiveBat) {
            Settings.Global.putInt(mContext.getContentResolver(),
                    Settings.Global.AGGRESSIVE_IDLE_ENABLED, mAggresiveIdleBatteryState);

            Settings.Global.putInt(mContext.getContentResolver(),
                    Settings.Global.AGGRESSIVE_STANDBY_ENABLED, mAggresiveStandbyBatteryState);
        }
